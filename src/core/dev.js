...
dbg.innerHTML='<div id="dbgstat"></div><div style="margin-top:6px"><button data-a="food">+25 FOOD</button> <button data-a="mats">+25 MATS</button> <button data-a="ids">IDS</button></div><div style="margin-top:6px">SPEED <button data-a="x1">X1</button> <button data-a="x5">X5</button> <button data-a="x20">X20</button></div><div style="margin-top:6px"><button data-a="save">SAVE</button> <button data-a="wipe">WIPE</button></div>';
...
if(a==="x5")ts=5;
if(a==="x20")ts=20;
if(a==="ids")showIds=!showIds;
if(a==="save")save();
if(a==="wipe")wipe();
});
function toggleDbg(){dbg.hidden=!dbg.hidden;if(dbg.hidden){ts=1;showIds=false;}}
if(location.hash.includes("debug"))dbg.hidden=false;
if(location.hash.includes("ids"))showIds=true;
