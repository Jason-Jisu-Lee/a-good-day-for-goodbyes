...
function zeroDelta(){return {mats:0,light:0,pr:0};}
function beatsBegin(){beatCap=[];beatShown={mats:G.mats,light:G.light||0,pr:G.pr||0};}
...
if(k==="scrap"){ev.label="MATERIAL";ev.au="material";}
else if(k==="light"||k==="light2"){ev.label="LIGHT";ev.au="cache";}
...
beatCap.push({t,prev:{kind:t.kind,state:"owned",atk:false},crew:[],floats:[],out:"income",label:"+"+amt,au:"material",dram:false,delta:dl,seq:1,gkey:"ginc-"+res});
}
function grpLabel(g){const m=g.members[0];if(m.out==="income"){const tot=g.delta.mats+g.delta.light+g.delta.pr;return tot>0?"+"+tot:""+tot;}return m.label;}
...
g.members.push(e);
g.delta.mats+=e.delta.mats;g.delta.light+=e.delta.light;g.delta.pr+=e.delta.pr;
for(const f of e.floats)g.floats.push(f);
}else merged.push(e);
}
for(const k in groups)groups[k].label=grpLabel(groups[k]);
const byq=[[],[]];
for(const e of merged)byq[e.seq?1:0].push(e);
beatQ=[...shuffle(byq[0]),...shuffle(byq[1])];
beatCap=null;beatBo=boFire;beatI=0;beatT=0;beatLead=BEAT_LEAD;
}
function beatSettle(ev){
if(beatShown&&ev.delta){beatShown.mats+=ev.delta.mats;beatShown.light+=ev.delta.light;beatShown.pr+=ev.delta.pr;}
