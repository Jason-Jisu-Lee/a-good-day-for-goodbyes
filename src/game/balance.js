...
function taskDays(S,n){return Math.max(1,Math.min(DAYS_CAP,S-n+1));}
function taskRisk(S,n){const o=S-n+1-DAYS_CAP;return o>0?Math.min(RISK_CAP,Math.round(o*RISK_STEP*100)/100):0;}
function crewUseful(S,n){return n<1||taskRisk(S,n)>0||taskDays(S,n)>1;}
